...
@PIPELINES.register_module()
class LoadImageAroundFile:
    """Load an multi-channel image from file.
    ...
    def __call__(self, results):
        ...
        _img = np.zeros((img.shape[0], img.shape[1], self.num_channels * 3))
        prefix = filename.split('frame')[0] + 'frame'
        ...
        if not self.is_testing:
            ...
            saturation = False
            if random.randint(2):
                saturation = random.uniform(self.saturation_lower,
                                            self.saturation_upper)

            # random hue
            hue = False
            if random.randint(2):
                hue = random.uniform(-self.hue_delta, self.hue_delta)

            # random contrast
            contrast = False
            if random.randint(2):
                contrast = random.uniform(self.contrast_lower,
                                          self.contrast_upper)
            ...
            if random.randint(2):
                if_permute = True
                permute = random.permutation(3)

        for i in range(self.num_channels):  # 3
            offset = i - (self.num_channels // 2)
            id = idx + offset
        ...
        results['img'] = _img
        results['img_shape'] = _img.shape
        results['ori_shape'] = _img.shape
        results['img_fields'] = ['img']
        return results
